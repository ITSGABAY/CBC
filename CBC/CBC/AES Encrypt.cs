        ...
        private static readonly byte[,] sBox =
{
        ...
        private static byte[] shiftRows(byte[] block)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            row2[currentIndex] = block[i * 4 + 1];
            currentIndex = i - 2;
            if (currentIndex < 0)
            {
                currentIndex = 4 + currentIndex;
            }
            row3[currentIndex] = block[i * 4 + 2];
            currentIndex = i - 3;
            if (currentIndex < 0)
            {
                currentIndex = 4 + currentIndex;
            }
            row4[currentIndex] = block[i * 4 + 3];
        }

        for (int i = 0; i < 4; i++)
        {
            newBlock[4 * i] = block[4 * i];
        ...
        private static byte[] mixColumns(byte[] block)
    {
        ...
        private static Byte[] addRoundKey(Byte[] block , Byte[] key) // good
        {
            Byte[] newBlock = new Byte[16];
            for(int i=0; i<16; i++)
            {
        ...
        private static Byte[] createKey(Byte[] key, int round)
        {
        ...
        private static Byte[] subBytes(Byte[] block) //good
        {
